...
class Player:
    def __init__(self, deck, name):
        ...
        hand_list = []
        for i in range(5):
            hand_list.append(self.deck.draw())
        self.hand = hand_list
...
class TutorCard(Card):
    ...
    def power(self, opponent_card):
        """
        override Card's power method to return -float('inf')
        """
        return -float('inf')
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand) > 0:
            ...
            for i in range(len(player.hand)):
                power_value.append(player.hand[i].power(opponent_card))
            index = power_value.index(max(power_value))
