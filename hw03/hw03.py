...
def pingpong(n):
    ...
    def helper_1(index, value):
        if index == n:
            return value
        else:
            if index % 8 == 0 or num_eights(index) != 0:
                return helper_2(index + 1, value - 1)
            else:
                return helper_1(index + 1, value + 1)

    def helper_2(index, value):
        if index == n:
            return value
        else:
            if index % 8 == 0 or num_eights(index) != 0:
...
def count_coins(change):
    ...
    def helper_change(n, m):
        ...
        if n == 0:
            ...
            return helper_change(n - m, m) + helper_change(n, next_smaller_coin(m))

    change = change - change % 5
    return helper_change(change, 25)
