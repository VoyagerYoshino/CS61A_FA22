...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.is_container and place.ant.can_contain(self) is False:
                assert place.ant is None, 'Two ants in {0}'.format(place)
            if place.ant.is_container ^ self.is_container:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.turns_to_chew == 0 and len(current_bees) != 0:
            random_bee(current_bees).reduce_health(random_bee(current_bees).health)
...
class TankAnt(ContainerAnt):
    """ ContainerAnt that protects an ant in its place and also deals 1 damage to all bees in its place each turn"""
    name = 'Tank'
    damage = 1
    food_cost = 6
    implemented = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if gamestate.queen_presence:
            return None
        else:
            gamestate.queen_presence = True
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        super(QueenAnt, self).action(gamestate)
        next_place = self.place.exit
        while next_place is not None:
            if next_place.ant is None:
                pass
            else:
                next_place.ant.double()
                if next_place.ant.is_container and next_place.ant.ant_contained is not None:
    ...
    def remove_from(self, place):
        if place.ant is None:
            assert False, '{0} is not in {1}'.format(self, place)
        elif place.ant is not None and place.ant is not self:
...
class SlowThrower(ThrowerAnt):
    ...
    damage = 0
    implemented = True
    state_dict = {}  # Change to True to view in the GUI
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        self.state_dict[target] = 1

    def action(self, gamestate):
        super(SlowThrower, self).action(gamestate)
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2):
        ...
        queen_presence -- whether a queen Ant in play or not
        """
        self.time = 0
        self.food = food
