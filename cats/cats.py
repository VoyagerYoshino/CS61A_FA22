...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if word == typed_word:
            ...
            difference.append(diff_function(typed_word, word, limit + 1))
...
def fastest_words(match):
    ...
    times = list(map(list, zip(*get_all_times(match))))
    for i in word_indices:
        result[times[i].index(min(times[i]))].append(get_word(match, i))
    return result
    # END PROBLEM 10
