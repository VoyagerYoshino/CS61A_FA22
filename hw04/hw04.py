...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return balanced(left_part) and balanced(right_part) and total_weight(left_part) * length(
            left(m)) == total_weight(right_part) * length(right(m))
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        return tree(lokis_replacement if str(label(t)) == 'loki' else label(t))
    else:
